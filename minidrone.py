...
class WriterThread(StoppableThread):
    ...
    def __init__(self, drone):
        StoppableThread.__init__(self)
        self.drone = drone
        self.gatt = pexpect.spawn('/usr/bin/gatttool -b ' + drone.mac + ' -I -t random', echo=False)
...
class MiniDrone(object):
    ...
    def setup_time(self):
        times = time_bin()
        for i in range(1, 3):
            self.send_ref('0004' + ('%02x' % i) + '00' + str(times[i-1]) + '00')
...
def time_bin():
    return [binascii.hexlify(str.encode(t)) for t in time.strftime("%Y-%m-%d|T%H%M%S%z", time.localtime()).split('|')]
